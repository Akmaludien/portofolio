...
export default function HomePage() {
  ...
  return <>
    ...
    <section className="discipline-band" aria-label="Areas of focus" data-testid="discipline-band">{disciplines.map(({ icon: Icon, name }, index) => <div key={name} data-testid={`discipline-${index + 1}`}><Icon aria-hidden="true" /><span>{name}</span></div>)}</section>
    <section id="projects" className="section-shell" data-testid="selected-projects-section"><div className="section-heading"><div><p className="eyebrow" data-testid="projects-eyebrow">Selected work / 02</p><h2 data-testid="projects-heading">Flagship project records</h2></div><p data-testid="projects-description">Only project identities supported by the approved portfolio record are presented.</p></div><div className="project-grid">{projects.map((project) => <ProjectCard key={project.slug} project={project} />)}</div></section>
